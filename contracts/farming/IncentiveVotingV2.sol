    ...
    OwnableUpgradeable
{
    ...
    mapping(address => mapping(uint256 => uint256[65535]))
        public userExternalPidVotes;
    ...
    function setFarming(IFarming _farming) external virtual {
        require(
            address(farming) == address(0),
            "farming address can be set only once"
        );
        farming = _farming;
  ...
  */
    // pool id 0 - vote - 30
    // pool id 1 - vote - 60
    function vote(Pool[] calldata _pids, uint256[] calldata _votes)
        external
        virtual
    {
        ...
        for (uint256 i = 0; i < _pids.length; i++) {
            ...
            if (pid.isExternal) {
                require(
                    externalPoolById[pid.id] != address(0),
  ...
  */
    ...
    function distributeRewards(uint256 _week) external virtual returns (bool) {
        require(
            !externalDistributionDone[_week],
            "already distributed for this week"
        );
        ...
        uint256 rewards = (totalPidRewards[_week] *
            currentWeekTotalExternalPidVotes) /
        ...
        for (uint256 _pid; _pid < approvedExternalPids.length; ++_pid) {
            ...
            uint256 pidReward = (rewards * votes) /
                (currentWeekTotalExternalPidVotes * WEEK);
            address externalPool = externalPoolById[_pid];
            if (
                rewardToken.allowance(address(this), externalPool) < pidReward
            ) {
    ...
    function addExternalPool(address _externalPool)
        external
        virtual
        onlyOwner
        returns (uint256)
    ...
    function removeReward(uint256 week, uint256 amount)
        external
        virtual
        onlyOwner
    {
        ...
        require(
            currentWeek < week,
            "You can remove rewards starting from the next week"
        );
        uint256 totalAmount = totalPidRewards[week] - amount;
        totalPidRewards[week] = totalAmount;
